...
export function buildPaperClouds(scene, { zMin = -380, zMax = 90 } = {}) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    sp.position.set((Math.random() - 0.5) * 700, 85 + Math.random() * 70, zMin + Math.random() * (zMax - zMin))
...
export function buildPaperSun(scene) {
  ...
  sun.position.set(240, 200, -430)
  scene.add(sun)
}

export function buildPaperMountains(scene, { cx = 0, cz = -145, rNear = 700, rFar = 880 } = {}) {
  ...
  // 两圈纸板山围成立体书舞台
  for (const [tex, r, w, h, n, off] of [[far, rFar, 640, 280, 8, 0.4], [near, rNear, 500, 220, 8, 0]]) {
    for (let i = 0; i < n; i++) {
      const ang = (i / n) * Math.PI * 2 + off
      const m = cutoutMesh(tex, w, h)
      m.position.set(cx + Math.cos(ang) * r, h * 0.38, cz + Math.sin(ang) * r)
      m.lookAt(cx, h * 0.38, cz)
      scene.add(m)
    }
  }
}
